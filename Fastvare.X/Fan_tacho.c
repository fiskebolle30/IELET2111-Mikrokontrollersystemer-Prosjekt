...
tacho_init()
{
    ...
    EVSYS.USERTCB0COUNT = 1; //Set TCB0 to accept events from channel (1-1) = ch.0 into the count event user.
    EVSYS.USERTCB1COUNT = 2; //Set TCB0 to accept events from channel (2-1) = ch.1 into the count event user.
    
    
    //Setup TCB2 as periodic event to capture and then reset both TCB0 and TCB1:
